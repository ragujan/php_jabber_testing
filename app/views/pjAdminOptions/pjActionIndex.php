	...
	if (isset($tpl['arr']))
	{
		if (is_array($tpl['arr']))
		{
			...
			if ($count > 0)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					if ($tpl['arr'][$i]['tab_id'] != 1 || (int) $tpl['arr'][$i]['is_visible'] === 0 || $tpl['arr'][$i]['key'] == 'o_theme') continue;
					$rowClass = NULL;
					$rowStyle = NULL;
					...
					<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
						<td>
							<?php
							if($tpl['arr'][$i]['key'] == 'o_email_address'){
								?>
								<span class="title-tooltip"><?php __('opt_' . $tpl['arr'][$i]['key']);?></span>&nbsp;<a href="#" class="pj-form-langbar-tip center-langbar-tip" title="<?php __('lblOptionEmailTip'); ?>"></a>
								<?php
							}else{
