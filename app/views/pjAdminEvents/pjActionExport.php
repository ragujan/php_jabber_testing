...
if (isset($tpl['status']))
{
	...
	<div class="ui-tabs ui-widget ui-widget-content ui-corner-all b10">
		<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
			...
			<li class="ui-state-default ui-corner-top ui-tabs-active ui-state-active"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminEvents&amp;action=pjActionExport"><?php __('lblExport'); ?></a></li>
		</ul>
	</div>
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminEvents&amp;action=pjActionExport" method="post" id="frmExportMovies" class="form pj-form">
		<input type="hidden" name="movie_export" value="1" />
		<p>
			<label class="title"><?php __('lblMovie'); ?></label>
			<span class="inline_block">
				<select name="event_id" id="event_id" class="pj-form-field w200">
					<option value="0">-- <?php __('lblAll');?> --</option>
					<?php
					foreach ($tpl['event_arr'] as $k => $v)
					{
						?><option value="<?php echo $v['id']; ?>"<?php echo isset($_POST['event_id']) && $_POST['event_id'] == $v['id'] ? ' selected="selected"' : null; ?>><?php echo pjSanitize::html($v['title']); ?></option><?php
					}
					?>
				</select>
			</span>
		...
		if(isset($_POST['type']) && $_POST['type'] == 'feed') 
		{
			...
			<div class="abFeedContainer">
				<br/>
				<?php pjUtil::printNotice(__('infoMoviesFeedTitle', true), __('infoMoviesFeedDesc', true)); ?>
				<span class="inline_block">
					<textarea id="movies_feed" name="movies_feed" class="pj-form-field h80" style="width: 726px;"><?php echo PJ_INSTALL_URL; ?>index.php?controller=pjAdminEvents&amp;action=pjActionExportFeed&amp;format=<?php echo$_POST['format']; ?>&amp;event_id=<?php echo $_POST['event_id'];?>&amp;type=<?php echo $_POST['period'] == 'next' ? '1' : '2'; ?>&amp;period=<?php echo $_POST['period'] == 'next' ? $_POST['coming_period'] : $_POST['made_period']; ?>&amp;p=<?php echo isset($tpl['password']) ? $tpl['password'] : null;?></textarea>
				</span>
			</div>
			<?php
		} 
